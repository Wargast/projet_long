...
def numpy_census_transform(im: np.ndarray, block_size: int) -> np.ndarray:
    (h, l) = im.shape
...
def naive_census_matching(
    ...
    bit_strings_l = numpy_census_transform(im_left, block_size)

    # calculate the bitstrings for the second image
    bit_strings_r = numpy_census_transform(im_right, block_size)
    ...
    for u in tqdm(range(h1_cropped)):
        for v1 in range(l1_cropped):
            for d in range(max_disparity + 1):
                v2 = v1 + d
                if v2 < l2_cropped:
                    bit_string_r = bit_strings_r[u, v1, :]
                    bit_string_l = bit_strings_l[u, v2, :]

                    # matching error : hamming distance between the two bistrings
                    cost = np.count_nonzero(bit_string_r != bit_string_l)
...
if __name__ == "__main__":
    ...
    # i = time.perf_counter()
    # error_map_python_l, error_map_python_r = naive_census_matching(
    #     im_left, im_right, block_size, max_disparity
    # )
    # j = time.perf_counter()
    # print(f"temps de calcul census matching python {j - i} s")

    # g = time.perf_counter()
    # error_map_numpy_l, error_map_numpy_r = census_matching_numpy(
    #     im_left, im_right, block_size, max_disparity
    # )
    # h = time.perf_counter()
    ...
    # if np.array_equal(error_map_numpy_l, error_map_python_l) and np.array_equal(error_map_numpy_r, error_map_python_r):
    #     print("les cartes d'erreurs sont bien égales")
    # else:
    #     raise ValueError("Les cartes d'erreurs n'ont pas la même valeur")
